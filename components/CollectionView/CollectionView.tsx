import { Avatar, Box, Chip, Grid, TextField, useMediaQuery, useTheme } from '@material-ui/core';
...
const CollectionView = () => {
  ...
  const [sortOrder, setSortOrder] = React.useState([
    { rarity: 'Backup', image_url: '/images/bronze-icon.svg', isEnabled: false },
    { rarity: 'Starter', image_url: '/images/silver-icon.svg', isEnabled: false },
    { rarity: 'Star', image_url: '/images/gold-icon.svg', isEnabled: false },
    { rarity: 'All-Pro', image_url: '/images/ruby-icon.svg', isEnabled: false },
    { rarity: 'Legend', image_url: '/images/sapphire-icon.svg', isEnabled: false },
    { rarity: 'Specialty', image_url: '/images/diamond-icon.svg', isEnabled: false }
  ]);
  ...
  const getFilterResults = (searchTerm, enabledChips) => {
    ...
    const onlySpecialtyCards = enabledChips.length === 1 && enabledChips[0].rarity === 'Specialty';
    ...
    if (!searchTerm && enabledChips) {
      ...
      if (includesSpecialtyCards) {
        return mockCards.filter((card) => enabledChipRarities.includes(card.rarity) || !ARRAY_OF_RARITIES.includes(card.rarity))
      }
      else {
        return mockCards.filter((card) => enabledChipRarities.includes(card.rarity))
      }
    }

    if (searchTerm && !enabledChips) {
      return mockCards.filter(cards =>
      ...
      if (includesSpecialtyCards) {
        return mockCards.filter(card =>
          (card.player_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
            card.player_team.toLowerCase().includes(searchTerm.toLowerCase())) && (
            enabledChipRarities.includes(card.rarity) || !ARRAY_OF_RARITIES.includes(card.rarity)))
      }
      return mockCards.filter(card =>
        (card.player_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          card.player_team.toLowerCase().includes(searchTerm.toLowerCase())) &&
  ...
  const filterBySpecialty = (hasSearchTerm) => {
    if (!hasSearchTerm) {
      return mockCards.filter((card) => !ARRAY_OF_RARITIES.includes(card.rarity));
    }
  ...
  return (
    <div className={classes.container}>
      ...
      <Box className={classes.chipList} component="div" whiteSpace="nowrap" overflow={mdUp ? "auto" : "scroll"} >
        {
          sortOrder.map((rarityFilter, index) => {
            return (<Chip key={rarityFilter.rarity} className={classes.rarityChip} variant={rarityFilter.isEnabled ? 'default' : 'outlined'} label={rarityFilter.rarity} avatar={<Avatar src={rarityFilter.image_url} />} onClick={() => handleChipClick(rarityFilter, index)} />)
          })
        }
      </Box>
      <Autocomplete
        ...
        renderInput={(params) => <TextField {...params} label="Enter player name" variant="outlined" />}
      ...
      <Grid className={classes.collectionContainer} container spacing={1}>
        {
          getFilterResults(searchTerm, enabledChips).map((card) => {
            console.log(card)
            return (
              <PlayerCard key={card.rarity + card.player_name} card={card} currentCard={currentCard} handleOpenCard={handleClickOpen} handleCloseCard={handleClose} open={open} />
            )
          })
        }
      </Grid>
