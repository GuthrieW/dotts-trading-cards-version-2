    ...
    <div className={classes.packViewerContainer}>
      <div className={classes.packContainer}>
        {props.map(({ x, y, rot, scale }, i) => (
          <animated.div className={classes.cardContainer} key={i} style={{ transform: interpolate([x, y], (x, y) => `translate3d(${x}px,${y}px,0)`) }}>
            {/* This is the card itself, we're binding our gesture to it (and inject its index so we know which is which) */}
            <animated.div className={classes.card} {...bind(i)} style={{ transform: interpolate([rot, scale], trans), backgroundImage: `url(${cards[i].image_url})` }} />
          </animated.div>
        ))}
      </div>
    </div>
