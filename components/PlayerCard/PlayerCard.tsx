...
const PlayerCard = ({ card, currentCard, handleOpenCard, handleCloseCard, open }) => {
  ...
  return (
    <Grid key={card.player_name} item xs={6} md={4} lg={3}>
      ...
      >
        <DialogContent>
          {currentCard && <img className={classes.cardContainer} src={currentCard.image_url} />}
        </DialogContent>
        {currentCard && <DialogTitle id="alert-dialog-title">{currentCard.player_name} - {currentCard.rarity}</DialogTitle>}
      </Dialog>
    </Grid>);
}
