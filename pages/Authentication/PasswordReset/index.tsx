  ...
  const updateCanSubmit = () => {
    ...
    if (passwordExists && confirmPasswordExists) {
      ...
      if (strength === 'Strong') {
        ...
        setError(
          'Password not strong enough. Must be at least 10 characters long and include 1 uppercase, 1 lowercase, 1 special character, 1 number.'
        )
        setCanSubmit(false)
      }
    } else {
  ...
  const handleSubmit = async () => {
    ...
    if (canSubmit) {
      ...
      const result = await axios({
        method: 'post',
        url: `/api/v1/authorization/resetPassword`,
        data: {
